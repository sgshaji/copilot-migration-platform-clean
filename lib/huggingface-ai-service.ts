...
export class HuggingFaceAIService {
  ...
  public static getInstance(): HuggingFaceAIService {
...
Generate specific scenarios that show dramatic business value improvements:`
  ...
  async generateBusinessImpactAnalysis(botAnalysis: BotAnalysis, deltaScenarios: DeltaScenario[]): Promise<any> {
    if (this.hasApiKey && this.hf) {
      try {
        const prompt = `Calculate business impact and ROI for transforming ${botAnalysis.name} to Microsoft Copilot agent. Consider time savings, efficiency gains, and revenue opportunities.`
        ...
        console.log("🤗 AI-generated business impact analysis")
      } catch (error) {
        console.warn("Business impact AI generation failed:", error)
      }
    }
    ...
    const scenarioTemplates = {
      HR: [
        ...
        },
        {
          ...
          description: "AI predicts staffing needs and identifies skill gaps using machine learning on historical data",
          currentLimitation: "Manual, spreadsheet-based workforce planning with quarterly reviews",
          copilotCapability: "AI-powered workforce analytics with predictive modeling and automated recommendations",
          ...
          implementationSteps: [
            "Deploy workforce analytics platform",
            "Connect to HRIS and performance systems",
            "Implement predictive modeling",
            "Create automated reporting engine",
          ],
          technicalRequirements: ["Azure Machine Learning", "Power Platform integration", "HRIS system connectors"],
        },
      ],
      IT: [
        {
  ...
  private getIntelligentBusinessImpact(botAnalysis: BotAnalysis): any {
    const domain = this.inferDomain(botAnalysis)
    const complexity = botAnalysis.intents.length * 0.1
